...
--Daedric Relics, Grendal’s Diagram
local s,id=GetID()
...
function s.rfilter(c,e,tp)
	...
	return #pg<=0 and c:IsRitualMonster() and c:IsSetCard(0x718) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_RITUAL,tp,true,false) and Duel.IsExistingMatchingCard(s.exfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c:GetCode())
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local rg=Duel.SelectMatchingCard(tp,s.rfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,1,nil,e,tp)
...
function s.spfilter(c,e,tp)
	...
	return c:IsSetCard(0x718) and (c:IsLocation(LOCATION_HAND) or c:IsFaceup())
