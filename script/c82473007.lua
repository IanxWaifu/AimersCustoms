...
function s.getseteffect(tc)
	...
	local em=tc:IsHasEffect(GENOSYNX_HAND_ACT)
	if em then
	...
	for _,eff in ipairs(effs) do
		if eff:GetCode()==GENOSYNX_HAND_ACT then
...
function s.sstrapfilter(c,tp)
	...
	return Duel.IsPlayerCanSpecialSummonMonster(tp,c:GetCode(),0,TYPE_EFFECT|TYPE_MONSTER|TYPE_SPIRIT,2000,2000,4,RACE_BEAST,ATTRIBUTE_DARK)
end
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(s.sstrapfilter,tp,LOCATION_REMOVED|LOCATION_GRAVE,0,1,nil,tp) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.sstrapfilter,tp,LOCATION_REMOVED|LOCATION_GRAVE,0,1,1,nil,tp)
	local tc=g:GetFirst()
	if tc then
