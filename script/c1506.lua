function c1506.initial_effect(c)
	--synchro summon
		Synchro.AddProcedure(c,aux.FilterBoolFunction(Card.IsRace,0x100000),1,1,Synchro.NonTuner(Card.IsRace,0x100000),3,99)
	c:EnableReviveLimit()
...
function c1506.filter(c,e,tp)
	return c:IsRace(RACE_PSYCHIC) and c:IsType(TYPE_SYNCHRO) and c:IsFaceup() and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
