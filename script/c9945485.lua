...
--activate
function c9945485.actfilter(c,e,tp)
...
function c9945485.actg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return loc>0 and Duel.IsExistingMatchingCard(c9945485.actfilter,tp,loc,0,1,nil,e,tp) end
end
function c9945485.acop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if Duel.GetLocationCount(tp,LOCATION_SZONE)>0 then loc=LOCATION_HAND+LOCATION_SZONE end
 	if Duel.GetLocationCount(tp,LOCATION_SZONE)==0 and Duel.GetLocationCount(tp,LOCATION_FZONE)==0 then loc=LOCATION_SZONE end
  	if loc==0 then return false end
    local g=Duel.GetMatchingGroup(c9945485.actfilter,tp,loc,0,nil,e,tp)
