...
function s.spcon(e,c)
	...
	return Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0 and Duel.IsCanRemoveCounter(c:GetControler(),1,1,COUNTER_ICE,7,REASON_RULE+REASON_RITUAL)
end
function s.spop(e,tp,eg,ep,ev,re,r,rp,c)
	Duel.RemoveCounter(tp,1,1,COUNTER_ICE,7,REASON_RULE+REASON_RITUAL)
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:GetCounter(COUNTER_ICE)>0 then
		if Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)~=0 and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then 
