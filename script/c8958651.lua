...
function s.desfilter(c)
	...
	return c:IsSetCard({SET_MAGICAL_MUSKET,SET_ZEFRA}) and c:IsAbleToHand()
end
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.desfilter,tp,LOCATION_REMOVED|LOCATION_GRAVE,0,1,nil) end
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.Destroy(c,REASON_EFFECT)>0 then
		...
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.desfilter),tp,LOCATION_REMOVED|LOCATION_GRAVE,0,1,1,nil)
		if #g>0 then
