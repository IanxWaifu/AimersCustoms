...
function c9945310.spfilter1(c,tp)
	return c:IsSetCard(0x204F) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() 
	and Duel.IsExistingMatchingCard(c9945310.spfilter2,tp,LOCATION_GRAVE,0,1,c,tp)
end
function c9945310.spfilter2(c)
	return c:IsSetCard(0x2050) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost()
end
function c9945310.spcon(e,c,tp)
	if c==nil then return true end
	local tp=c:GetControler()
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return false end
	return Duel.IsExistingMatchingCard(c9945310.spfilter1,tp,LOCATION_GRAVE,0,1,nil,tp)
end
