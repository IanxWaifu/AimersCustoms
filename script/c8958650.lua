--Scripted by Aimer
--Zefracalibre, the Magical Musketerror
local s,id=GetID()
function s.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_TOHAND+CATEGORY_DESTROY+CATEGORY_SEARCH)
...
function s.spfilter(c,e,tp)
	return c:IsSetCard({SET_MAGICAL_MUSKET,SET_ZEFRA}) and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.desfilter(c)
	return c:IsSetCard(SET_MAGICAL_MUSKET) and c:IsAbleToHand() and c:IsSpellTrap()
end
