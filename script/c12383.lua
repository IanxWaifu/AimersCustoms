...
function c12383.spfilter1(c,tp)
	return c:IsSetCard(0x3052) and c:IsType(TYPE_SYNCHRO) and c:IsAbleToDeckOrExtraAsCost() and c:IsFaceup() and c:IsCanBeFusionMaterial()
		and Duel.IsExistingMatchingCard(c12383.spfilter2,tp,LOCATION_REMOVED,0,2,c:GetCode(),tp)
end
function c12383.spfilter2(c,tp,code)
	return c:IsSetCard(0x3052) and c:IsFaceup() and c:IsType(TYPE_MONSTER) and not c:IsType(TYPE_SYNCHRO) and c:IsCanBeFusionMaterial() and c:IsAbleToDeckOrExtraAsCost() and not c:IsCode(code)
end
function c12383.sprcon(e,c,tp)
	if c==nil then return true end
	local tp=c:GetControler()
	local g=Duel.GetMatchingGroup(c12383.spfilter1,tp,LOCATION_REMOVED,0,code,tp) 
