...
function s.filter1(c,e,tp)
    return ((c:IsRace(RACE_WARRIOR) and c:IsType(TYPE_PENDULUM) and c:IsLocation(LOCATION_DECK) and c:IsAbleToRemove()) or (c:IsCode(999180) and c:IsLocation(LOCATION_ONFIELD) and c:IsAbleToRemove())) and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_DECK,0,1,nil,e)
...
function s.filter2(c,e,tp,code)
    return (code~=999180 and ((c:IsDestructable(e) and c:IsRace(RACE_SPELLCASTER) and c:IsLocation(LOCATION_DECK) and c:IsType(TYPE_PENDULUM)) or (c:IsCode(999180) and c:IsLocation(LOCATION_ONFIELD) and c:IsDestructable(e)))) or
           (code==999180 and not c:IsCode(999180) and c:IsDestructable(e) and c:IsRace(RACE_SPELLCASTER) and c:IsType(TYPE_PENDULUM) and c:IsLocation(LOCATION_DECK))
end
