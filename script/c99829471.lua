...
function s.initial_effect(c)
	...
	e2:SetValue(function(e,c) if c:IsSetCard(SET_SYLVESTRIE) then return 500 end return -500 end)
	c:RegisterEffect(e2)
	--Battle Phase Entry Intercept
...
function s.bpcon(e,tp,eg,ep,ev,re,r,rp)
    return Duel.GetTurnPlayer()~=tp and Duel.IsPhase(PHASE_BATTLE_START)
end

function s.bpop(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    local turn_player=Duel.GetTurnPlayer()
    local decision_player=1-turn_player
    if Duel.GetCurrentPhase()~=PHASE_BATTLE_START then return end
    if Duel.SelectYesNo(decision_player,aux.Stringid(id,0)) then
        -- YES: allow BP + owner draws 1
        Duel.Draw(c:GetControler(),1,REASON_EFFECT)
        return
    end
    -- NO: skip directly to End Phase
...
function s.rspop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(e:GetHandler():GetOwner(),LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,p,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(p,s.rspfilter,p,LOCATION_DECK,0,1,1,nil,e,tp)
	if #g>0 then
...
function s.drsptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,nil,1,1-e:GetHandler():GetOwner(),LOCATION_ONFIELD)
end

function s.drspfilter(c,exc)
	return c~=exc
end
...
function s.drspop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(p,s.drspfilter,p,LOCATION_ONFIELD,0,1,1,c,c)
	if #g>0 then
