...
function s.initial_effect(c)
	...
	e2:SetCategory(CATEGORY_SET+CATEGORY_REMOVE)
...
-- Set activation: flip monster face-down
...
function s.handtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(s.settrapfilter,tp,LOCATION_HAND|LOCATION_GRAVE|LOCATION_REMOVED,0,1,nil,ft) end
	e:SetLabel(2) -- MODE 2: activated from hand -> Set this card in EP
	Duel.SetOperationInfo(0,CATEGORY_SET,nil,1,tp,LOCATION_HAND|LOCATION_GRAVE|LOCATION_REMOVED)
...
function s.trapmonster(e,tp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 or not Duel.IsPlayerCanSpecialSummonMonster(tp,id,SET_GENOSYNX,TYPE_EFFECT|TYPE_MONSTER|TYPE_SPIRIT,1000,1000,4,RACE_BEAST,ATTRIBUTE_DARK) then return end
	...
	-- store mode for End Phase (1=return hand, 2=set itself)
